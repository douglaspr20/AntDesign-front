...
export function* getAllResourceResponsesSaga({ payload }) {
	...
	try {
		...
		if (response.status === 200) {
			yield put(responseActions.setAllResourceResponse(response.data.allSkillCohortResourceResponses));
		}
	} catch (error) {
		console.log(error);
	} finally {
...
export function* getResourceResponseSaga({ payload }) {
	...
	try {
		...
		if (response.status === 200) {
			yield put(responseActions.setResourceResponse(response.data.skillCohortResourceResponse));
		}
	} catch (error) {
		console.log(error);
	} finally {
...
export function* updateResourceResponseSaga({ payload }) {
	...
	try {
		...
		if (response.status === 200) {
			...
			yield put(responseActions.setResourceResponse(response.data.affectedRows));
		}
	} catch (error) {
		console.log(error);
		notification.error({
...
function* watchResponseSaga() {
	yield takeLatest(responseConstants.GET_ALL_RESOURCE_RESPONSES, getAllResourceResponsesSaga);
	yield takeLatest(responseConstants.CREATE_RESOURCE_RESPONSE, createResourceResponseSaga);
	yield takeLatest(responseConstants.GET_RESOURCE_RESPONSE, getResourceResponseSaga);
	yield takeLatest(responseConstants.UPDATE_RESOURCE_RESPONSE, updateResourceResponseSaga);
}
