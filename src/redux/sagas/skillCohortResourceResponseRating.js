...
function* getAllResourceResponseRatingSaga({ payload }) {
    ...
    try {
        ...
        if (response.status === 200) {
            yield put(responseRatingActions.setAllResponseRating(response.data.allSkillCohortResourceResponseRatings))
        }
...
function* watchResponseSaga() {
    yield takeLatest(responseRatingConstants.GET_ALL_RESPONSE_RATING, getAllResourceResponseRatingSaga)
    yield takeLatest(responseRatingConstants.UPSERT_RESPONSE_RATING, upsertResourceResponseRatingSaga)
}
