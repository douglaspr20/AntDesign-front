...
const SkillCohortCard = (props) => {
	...
	useEffect(() => {
		const dateToday = moment().tz('America/Los_Angeles').format('YYYY-MM-DD HH:mm:ssZ');

		if (dateToday > moment(startDate).tz('America/Los_Angeles').format('YYYY-MM-DD HH:mm:ssZ')) {
	...
	const randomId = `skill-cohort-description-${Math.floor(Math.random() * 1000)}`;
	...
	return (
		<div className="skill-cohort-card">
			<div className="skill-cohort-card-header">
				{image ? <img src={image} alt="header" /> : <img src={HARDCODED_COVER_PLACEHOLDER} alt="header" />}
			</div>
